...
int normal_attack_skill(struct character *p);//普攻，立刻生效。由zzb调用
void grow_attack_skill(struct character *p);//辅助攻击技能，立刻生效。由zzb调用
void unique_skill(struct character *p);//大招，立刻生效。由zzb调用


void defend_skill(struct character *p);//由zzb调用，主动使用防御技能，使用后下次攻击触发主动防御效果，否则触发被动防御效果
void defend_clpt(struct character *p, int atk);//由hrh调用，用于放在无线通信的中断回调里，用于在被攻击时响应对方。
